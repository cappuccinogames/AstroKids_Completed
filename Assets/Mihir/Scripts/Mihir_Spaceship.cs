...
public class Mihir_Spaceship : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        TouchManager.Instance.onTouchBegan += OnBegan;
        TouchManager.Instance.onTouchMoved += OnMove;
        TouchManager.Instance.onTouchEnded += OnEnded;
    }
    ...
    private void OnBegan(TouchInput touch)
    {

        transform.rotation = Quaternion.identity;
    }

    private void OnEnded(TouchInput touch)
    {
    ...
    private void OnMove(TouchInput touch)
    {
        Vector3 newPos = transform.position + touch.DeltaScreenPosition.y * Vector3.up * 0.15f;
        transform.position = Vector3.Lerp(transform.position, newPos, Time.deltaTime * _speed);
        verticalInput =touch.DeltaScreenPosition.y;
    }
