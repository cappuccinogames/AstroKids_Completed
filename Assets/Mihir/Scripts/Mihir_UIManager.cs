...
public class Mihir_UIManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_spaceship.transform.position.x > _answerAText.transform.position.x)
        {
            ...
            //    _levelManager.finishLevel();
                //_answerAText.transform.position += new Vector3(1000, 0, 0);
                if(_spaceship.transform.position.y > 2.3f && answer==1)//if (_spaceship.transform.position.y > divider1.transform.position.y && answer == 1)
                {
                    _levelManager.finishLevel();
                    //_answerAText.transform.position += new Vector3(1000, 0, 0);
                }
                else if(_spaceship.transform.position.y > -1.39f && _spaceship.transform.position.y < 1.85 && answer == 2)//else if (_spaceship.transform.position.y < divider1.transform.position.y && _spaceship.transform.position.y > divider2.transform.position.y && answer == 2)
                {
                    _levelManager.finishLevel();
                    // _answerAText.transform.position += new Vector3(1000, 0, 0);
                }
                else if (_spaceship.transform.position.y < -1.79f && answer == 3)//else if (_spaceship.transform.position.y < divider2.transform.position.y && answer == 3)
                {
                    _levelManager.finishLevel();
                    //_answerAText.transform.position += new Vector3(1000, 0, 0);
                }
